...
var mime = require('./node-staticp/mime');
var util = require('./node-staticp/util');
...
exports.Server = function (root, options) {
     ...
     */
    this.postscript = this.options.postscript || '';

    /**
     * @var {bool} isMultiDomain If true, files served will be in domain subfolders (i.e. {root}/{hostname}/{path}).
     */
    this.isMultiDomain = !!this.options.isMultiDomain;

    if ('serverInfo' in this.options) {
        this.serverInfo = this.options.serverInfo.toString();
    } else {
        this.serverInfo = 'node-staticp/' + exports.version.join('.');
    }
...
exports.Server.prototype.serveDir = function (pathname, req, res, finish) {
    ...
    fs.stat(htmlIndex, function (e, stat) {
        if (!e) {
            var files = [htmlIndex];
            if (that.postscript) { files.push(that.resolve(that.postscript, req)); }
            that.streamFiles(null, files, 200, {}, req, res, finish);
        } else {
            if (pathname in exports.indexStore) {
                that.streamFiles(pathname, exports.indexStore[pathname].files, 200, {}, req, res, finish);
            } else {
                ...
                fs.readFile(path.join(pathname, 'index.json'), function (e, contents) {
                    ...
                    that.streamFiles(pathname, index.files, 200, {}, req, res, finish);
                });
            }
        }
    });
...
exports.Server.prototype.streamFiles = function (pathname, files, status, headers, req, res, finish) {
    var that = this;
    util.mstat(pathname, files, function (e, stat) {
        if (e) { return finish(404, {}) }
        that.respond(pathname, status, headers, files, stat, req, res, finish);
    });
};

exports.Server.prototype.serveFile = function (pathname, status, headers, req, res, finish) {
    var that = this;
    ...
    if (that.postscript) {
        ...
        if (fileExtension === 'html') { files.push(that.resolve(that.postscript, req)); }
    }
    that.streamFiles(null, files, status, headers, req, res, finish);
    ...
    if (pathname.indexOf(that.root) === 0) {
        fs.stat(pathname, function (e, stat) {
            if (e) {
                ...
                that.serveFile(pathname, status, headers, req, res, finish);
...
exports.Server.prototype.resolve = function (pathname, request) {
    return path.resolve(path.join(this.root, (this.isMultiDomain && request ? url.parse('http://' + request.headers.host).hostname + '/' : ''), pathname));
};
